...
// static std::pair<FlexSkeletonHeader*, int> propSkeletons[] = {
//     { (FlexSkeletonHeader*)gDogSkel, DOG_LIMB_MAX },
//     { (FlexSkeletonHeader*)object_mm_Skel_0096E8, 16 },
// };

// static AnimationInfoS propAnimationInfo[] = {
//     { (AnimationHeader*)&gDogLyingDownLoopAnim, 1.0f, 0, -1, ANIMMODE_LOOP, -6 },
//     { (AnimationHeader*)&gDogWalkAnim, 1.0f, 0, -1, ANIMMODE_LOOP, -6 },
//     { (AnimationHeader*)&gDogRunAnim, 1.0f, 0, -1, ANIMMODE_LOOP, 0 },
//     { (AnimationHeader*)&object_mm_Anim_00B09C, 1.0f, 0, -1, ANIMMODE_LOOP, 0 },
//     { (AnimationHeader*)&object_mm_Anim_002238, 1.0f, 0, -1, ANIMMODE_LOOP, 0 },
//     { (AnimationHeader*)&object_mm_Anim_002238, 1.0f, 0, -1, ANIMMODE_LOOP, 0 },
// };
...
void DummyUpdate(Actor* actor, PlayState* play) {
    ...
    if (Anchor::Instance->clients.contains(clientId)) {
        if (Anchor::Instance->clients[clientId].sceneId == gPlayState->sceneId) {
            ...
            memcpy(&client.player.jointTableUpperBuffer, &client.upperJointTable, 159);

            client.player.maskObjectLoadState = 0;
            client.player.maskId = client.player.currentMask;
            client.player.currentMask = client.currentMask;
            client.player.rightHandType = client.rightHandType;
            client.player.leftHandType = client.leftHandType;
            client.player.currentShield = client.currentShield;
            client.player.sheathType = client.sheathType;
            client.player.heldItemAction = client.heldItemAction;
            client.player.heldItemId = client.heldItemId;
            client.player.itemAction = client.itemAction;
            client.player.stateFlags1 = client.stateFlags1;
            client.player.stateFlags2 = client.stateFlags2;
            client.player.stateFlags3 = client.stateFlags3;
            client.player.unk_B0C = client.unk_B0C;
            client.player.unk_B28 = client.unk_B28;
            client.player.unk_ACC = client.unk_ACC;
            Player_SetModels(&client.player, Player_ActionToModelGroup(&client.player, (PlayerItemAction)client.player.itemAction));
